...
- Rate limiting
- k8s cluster expansion scaling - _under construction_
</br>
...
When applying the **nic-vs-cafe-adv-routing.yaml** manifest file, we can see:
    ...
    - For the F5 CIS, we are creating a **TLSProfile** named **bridging-tls-profile-npl**, that will do TLS termination with mode bridging (**_termination: reencrypt_** on the manifest). For the clientside SSL, it will used the same **cafe-secret** secret cert & key, and for the serverside SSL it will used the preconfigured **serverssl-insecure-compatible** profile on BIGIP. Because using combination of clientside and serverside SSL profile, that is why the **reference** command on the manifest are using **hybrid** mode.
